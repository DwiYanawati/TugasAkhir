...
# 1. SET CONFIG UTAMA (Layout Wide & Sembunyikan Sidebar Secara Default)
st.set_page_config(
...
# 2. SUNTIKAN CSS PREMIUM (Navbar Tengah Atas & Kartu Kontainer Dashboard)
st.markdown("""
    <style>
    /* Background Global */
    .stApp {
    ...
    /* REKAYASA GAYA TAB MENJADI NAVBAR PREMIUM DI TENGAH ATAS */
    .stTabs [data-baseweb="tab-list"] {
        ...
        justify-content: center; /* Membuat menu presisi di tengah atas website */
        margin-bottom: 35px;
    }
...
# 3. HEADER UTAMA ATAS (HERO SECTION)
...
# 4. DEKLARASI NAVBAR TAB HORIZONTAL DI TENGAH ATAS WEBSITE
menu_tab1, menu_tab2, menu_tab3 = st.tabs([
    "🏠  Halaman Utama & Informasi", 
    "📤  Fitur Deteksi via Upload", 
    "🎥  Fitur Deteksi Real-Time"
])

# 5. CORE MODEL LOADING WITH CACHING
@st.cache_resource
def load_model():
    try:
        model = YOLO("best.pt")
...
if model is not None:
    ...
    with menu_tab1:
        # KARTU A: INFORMASI SISTEM & METRIK SKRIPSI
    ...
    with menu_tab3:
        ...
        with col_cam1:
            st.markdown('<div class="custom-card">', unsafe_allow_html=True)
            st.subheader("🎥 Video Stream Berbasis WebRTC")
            webrtc_streamer(
                key="yolov9-topnavbar-detection",
        ...
        with col_cam2:
            ...
            st.caption("""
            - Berikan izin akses kamera pada penjelajah web Anda (*Allow Camera*).
            - Atur posisi daun kedelai secara stabil di depan kamera untuk menghindari efek *motion blur*.
            - Perubahan pencahayaan lingkungan lahan berpengaruh langsung pada akurasi prediksi model.
            """)
            st.markdown('</div>', unsafe_allow_html=True)

else:
    st.error("Gagal memuat berkas bobot model 'best.pt'. Periksa kembali direktori repositori Anda.")
